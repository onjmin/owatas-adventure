/**
 * phina.js TypeScript type definitions
 * Borrowed from: https://github.com/negiwine/phina.js.d.ts
 * License: MIT
 */
declare namespace phina {
	...
	namespace util {
		...
		interface FlowStatic {
			...
			<T>(func: (resolve: (arg: T) => void, reject: () => void) => void, wait?: boolean): Flow<T>;
	...
	namespace asset {
		...
		type AssetLoaderAssetLoadFunction = (key: string, path: string) => util.Flow<any>;
	...
	namespace display {
		...
		interface SpriteStatic {
			...
			(image: string | asset.Texture | graphics.Canvas, width?: number, height?: number): Sprite;
		}
	...
	namespace game {
		...
		interface ManagerSceneStatic {
			new (options: { scenes: ManagerSceneData[]; startLabel?: string | number }): ManagerScene;
			(options: { scenes: ManagerSceneData[]; startLabel?: string | number }): ManagerScene;
		}
